...
static inline float Grad(int x, int y, int z, float dx, float dy, float dz)
{
    // Noise LUT is twice as long so no need for modulus
    // Also, this produce better irregularity thatn NoisePerm[ix + iy + iz]
    int h = NoisePerm[NoisePerm[NoisePerm[x] + y] + z];
    h &= 15;
    float u = h < 8 || h == 12 || h == 13 ? dx : dy;
    float v = h < 4 || h == 12 || h == 13 ? dy : dz;
    
    // Dot product between grads is just addition since
    // they only use -1, 0, 1 in their coordinates
    return ((h & 1) ? -u : u) + ((h & 2) ? -v : v);
}

static inline float NoiseWeight(float t)
{
    float t3 = t * t * t;
    float t4 = t3 * t;
...
public:
    ...
    // Composite noise to create turbulence
    static float Turbulence(const glm::vec3& point, int depth = 7)
    {
        float accum = 0.0f;
